...
public class VoidAttachmentContainerMenu extends BaseContainerMenu {
    ...
    public VoidAttachmentContainerMenu(
        int windowId,
        Player player,
        BlockPos pos,
        Direction dir,
        ...
        boolean fluidMode) {
        super(FPipesContainerMenus.VOID_ATTACHMENT.get(), windowId, player);

        addPlayerInventory(8, 111);

        int x = 44;
        int y = 19;
        for (int i = 1; i <= VoidAttachment.MAX_FILTER_SLOTS; ++i) {
            if (fluidMode) {
    ...
    public VoidAttachmentContainerMenu(int windowId, Player player) {
        this(windowId, player, BlockPos.ZERO, Direction.NORTH,
            BlacklistWhitelist.BLACKLIST, false,
            new ItemStackHandler(VoidAttachment.MAX_FILTER_SLOTS),
            new FluidInventory(VoidAttachment.MAX_FILTER_SLOTS), false);
    }
